...
export const RESUME_DATA: ResumeData = {
  ...
  skills: {
    product: [
      "AI 产品策略与 0→1 落地",
      "AI 能力边界与 fail-safe 设计",
      "Agent / ChatOps 场景化交付",
      "模型评测与上线门槛（Golden Dataset · QA Rubric）",
      "跨团队 AI 基建协同",
      "高风险合规场景产品化",
    ],
    ai: [
      "LLM 应用架构（RAG · Agent · Workflow）",
      "Agent 编排与 Tool Use（OpenClaw · Coze · 意图路由）",
      "多模型场景路由（Qwen / Claude / DeepSeek / Gemini）",
      "联网检索与可溯源 AI（OpenClaw · Tavily · 防幻觉）",
      "Prompt Engineering 与结构化输出",
      "LLM 成本/延迟优化（Semantic Cache · 规则分流）",
      "多模态文档理解（PDF · 材料入库）",
      "长任务 AI 体验（流式分章 · 进度可见）",
    ],
    engineering: [
      "Vibe Coding（Cursor / Claude Code）",
      "全栈交付（Next.js / TypeScript / Python / Node.js）",
      "Notion 数据中台 · 飞书 Open API",
      "Git / Vercel 部署",
    ],
  },
  work: [
    {
      company: "花旗金融信息服务 (Citi Financial Information Services)",
      role: "经理 | AI 产品负责人",
      period: "2021.07 – 2025.09",
      context:
        "前期 2 年参与 Pre-Trade 数据平台现代化重构（数据校验、Angular 迁移、API 重构），建立交易流程与 Trader 决策模式的系统理解。",
      highlights: [
        "【Pre-Trade AI 合规智能助手】主导 0→1 AI 产品转型，定位「合规副驾驶」（辅助决策 + fail-open 策略，不替代风控系统）。设计 L0 规则引擎 + L1 RAG 语义引擎混合架构：L0 处理约 80% 确定性违规（<50ms），L1 处理语义灰区（Hybrid Search BM25 0.4 + Dense 0.6，召回率 >98%）。",
        "引入 Semantic Cache（语义哈希 + Redis 24h），将约 40% 重复查询延迟从 ~1.5s 降至 ~200ms，月度 LLM 推理成本节省约 $120K。设计红黄绿三级信号 + Override 必填理由机制，构建「Trader 反馈→专家标注→模型优化」数据飞轮。",
        "P2 单 desk 试点 4 周（约 800–1200 笔 RFQ），误报率从约 20%–30% 降至 10%–15%，执行采纳率 65%–70%。推动大模型基建团队与应用团队建立分层问责协议（检索层→模型层→数据层逐层排查），通过 MVP API 方案（12→3 个 endpoint）打破排期僵局，2 周完成联调。",
        "【AI 合规知识库与法规情报系统】设计面向合规团队的「法规变化感知 + 可追溯问答」平台。RBAC 权限在检索阶段前置过滤，从架构层杜绝跨团队数据泄露；多模态文档解析链路（PyMuPDF→结构化布局分析→Gemini Vision）专项解决金融 PDF 跨页表格、嵌套结构抽取。",
        "回答强制附带法规原文引用（不允许无来源回答），设定 Golden Dataset 上线门槛（F1>90%）与专家复核机制。",
        "【Vantage Match Engine 推荐流优化】在覆盖 Credit/Mortgage/Short Term 三大资产类别的 Pre-Sales 平台上，将跨模块人工比对改造为单页实时推荐 + 匹配原因可视化，决策耗时 5–8min→1–2min，采用率首月 50%→次月 65–70%，此经验直接驱动后续 AI 合规助手的可解释推理链设计。",
      ],
    },
  ],
  projects: [
    {
      title: "InterviewOS | 全链路 AI 面试教练",
      link: "interview.mengxing-ai.it.com",
      summary:
      ...
      tech: "Next.js 15 · Vercel AI SDK · Qwen / Claude / DeepSeek · Notion API",
    },
    {
      title: "Job Engine | 全自动求职与背调引擎",
      link: "interview.mengxing-ai.it.com/job-analysis?tab=monitor",
      summary:
      ...
      bullets: [
        ...
        "设计飞书 ChatOps + OpenClaw Agent 意图路由：区分资讯查询、岗位采集、今日简报、公司背调；背调强制挂载 web_search/web_fetch，结论标注 URL 来源消除幻觉。",
        "Notion 作为唯一事实源：采集结果带评分与标签，可按入库时间生成 24h 简报；Web/Electron 看板支持筛选与背调长文阅读。",
        "飞书仅推送核心结论，长篇背调报告写入岗位页，兼顾移动阅读与深度复盘。",
      ],
      tech: "Python · Playwright / DrissionPage · DeepSeek · OpenClaw · 飞书 · Notion · Next.js · Electron",
    },
    {
      title: "费曼学习工具 | 深度学习引擎",
      link: "mengxing-ai.it.com/learning",
      summary:
    ...
    {
      title: "横纵分析法 | 深度研究引擎",
      link: "mengxing-ai.it.com/tools/hv-analysis",
      summary:
        "将「纵轴时间叙事 + 横轴竞品对照」产品化：定题后自动检索、分章流式生成万字研报，14 条军规 QA 与 Notion 历史舱归档。",
      bullets: [
        "研究流水线：Tavily 预检索素材→分章流式生成（纵向时间线 + 横向竞品矩阵）→洞察交汇章节，SSE 推送章节进度突破单次上下文上限。",
        "14 条规则质检未通过章节可定向重写，形成「生成—评估—修正」闭环，显著提升研报可用率与专业感。",
        "定稿经用户确认后归档 Notion 历史舱，支持导出 Markdown 与按时间检索元数据，把一次性对话变为可积累的研究资产。",
      ],
  ...
  education: [
    { degree: "软件工程 硕士", school: "中南大学", year: "2021" },
    { degree: "物联网工程 学士", school: "湖南农业大学", year: "2018" },
  ],
};
